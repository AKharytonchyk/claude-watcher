...
2. ✅ Provider badge groundwork — `AgentBrand.symbol` carried through to
   `AgentVM`; a quiet monochrome provider glyph renders on the row, shown only
   when >1 distinct provider is present (invisible while Claude-only).
